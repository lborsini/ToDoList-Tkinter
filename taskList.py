...
#Currying : retrasar la aplicacion de nuestra funcion
def complete(id):
    def _complete():
...
def renderTask():
    ...
    for i in range(0, len(rows)):
        ...
        p = Checkbutton(frame, text=description, fg=color, width=42, anchor='w', command=complete(id))
        p.grid(row=i, column=0, sticky='w')
        btn = Button(frame, text='Eliminar', command=remove(id))
        btn.grid(row= i, column=1)
        p.select() if completed else p.deselect()
...
btn = Button(root, text='My Task', padx= 5, pady= 5, command=addTodo)
